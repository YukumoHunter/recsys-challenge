...
from recsys_challenge.utils._polars import (
...
from recsys_challenge.utils._python import create_lookup_dict
...
def filter_minimum_negative_samples(
    ...
    """
    >>> from recsys_challenge.utils._constants import DEFAULT_CLICKED_ARTICLES_COL, DEFAULT_INVIEW_ARTICLES_COL
    >>> df = pl.DataFrame(
            {
...
def sample_article_ids(
    ...
    Examples:
    >>> from recsys_challenge.utils._constants import DEFAULT_INVIEW_ARTICLES_COL
    >>> df = pl.DataFrame(
            {
                "clicked": [
                    [1],
...
def remove_positives_from_inview(
    ...
    Examples:
    >>> from recsys_challenge.utils._constants import DEFAULT_INVIEW_ARTICLES_COL, DEFAULT_CLICKED_ARTICLES_COL
    >>> df = pl.DataFrame(
            {
                "user_id": [1, 1, 2],
                DEFAULT_CLICKED_ARTICLES_COL: [
